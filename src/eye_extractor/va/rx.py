...
MANIFEST_PAT = re.compile(
    ...
    r'(?:add(?:\W*power)?\W*(?P<od_add>\d+(?:\.\d+)?)\W*)?'
    r'(?:(BVA|best correct vision)\W*20/\s*(?P<od_denominator_2>\d+)\s*(?P<od_correct_2>[+-]\d+(?:\.\d+)?)?\s*)?'
    ...
    r'(?:add(?:\W*power)?\W*(?P<os_add>[+-]?\d+(?:\.\d+)?)\W*)?'
    r'(?:(BVA|best correct vision)\W*20/\s*(?P<os_denominator_2>\d+)\s*(?P<os_correct_2>[+-]\d+(?:\.\d+)?)?\s*)?',
    re.I
)
